...
to initialize ; sets up the colonies, initial flowers, grass, and gives each nest 10 ants
  ...
  [
    ...
    set garrison -1
    set hole? false
    set intelligence "neither"
  ]

  set garrison-1 patches at-points [
    [-1 -1] [-1 0] [-1 1]
    [0 -1] [0 0] [0 1]
    [1 -1] [1 0] [1 1]
  ]
  make-garrison 1 garrison-1 patch 0 0
  make-hole 2
end
;; the model loop ;;
...
to move-marines ;moves marines
  ask marines
  [
    ifelse fighting? = false [
      any-body-here-to-fight?
      ifelse patrol? = false [
      ifelse any? patches in-radius 0.5 with [garrison = [garrison-number] of myself] [
        wiggle
        fd 1
      ]
      [
...
to make-garrison [number garrison-patches spawn-point]
  ask garrison-patches [
    set garrison number
    set hole? false
    set intelligence "US"
    set pcolor blue
  ...
  ask spawn-point [
    sprout-marines 15 [
      ...
      set garrison-number number
      set spawn spawn-point
      set fighting? false
    ]
  ]
end
...
to make-hole [number]
  repeat number [
    ask one-of patches with [garrison >= 0] [
      ask one-of patches with [pcolor = grey] in-radius 3 [
        set pcolor brown
        set hole? true
        sprout-nvas 5 [set color red]
      ]
    ]
  ]
end
...
to any-body-here-to-fight? ; turtle procedure. Checks for other ants nearby of the other team, and makes sure the list isn't empty
  ...
  if any? other nvas in-radius 1
  [ ; checks to make sure there are some other ants here
    ifelse any? other marines in-radius 1
...
to attack ; turtle procedure that checks if the ant it fighting, and the prey ant isn't of this ants colony, then  run away, and try to kill the ant, or stop fighting
  if fighting? = true
  [
    set enemy one-of nvas-here

    ifelse enemy != nobody
    [
      ask enemy [ face myself ]
